...
Changes (breaking or otherwise) in v1.0.0 can be found [here](https://github.com/diotoborg/alias-amet/issues/935).
...
* (Optional) [`cpu-features`](https://github.com/mscdex/cpu-features) is set as an optional package dependency (you do not need to install it explicitly/separately from `@diotoborg/alias-amet`) that will be automatically built and used if possible. See the project's documentation for its own requirements.
  * This addon is currently used to help generate an optimal default cipher list

## Installation

    npm install @diotoborg/alias-amet
...
const { Client } = require('@diotoborg/alias-amet');
...
socks.createServer((info, accept, deny) => {
  // NOTE: you could just use one @diotoborg/alias-amet client connection for all forwards, but
...
const { Client, HTTPAgent, HTTPSAgent } = require('@diotoborg/alias-amet');
...
const { utils: { parseKey }, Server } = require('@diotoborg/alias-amet');
...
new @diotoborg/alias-amet.Server({
  hostKeys: [readFileSync('host.key')]
}, (client) => {
  console.log('Client connected!');
...
```js
const { utils: { generateKeyPair, generateKeyPairSync } } = require('@diotoborg/alias-amet');
...
`require('@diotoborg/alias-amet').Client` is the **_Client_** constructor.

`require('@diotoborg/alias-amet').Server` is the **_Server_** constructor.

`require('@diotoborg/alias-amet').utils` is an object containing some useful [utilities](#utilities).

`require('@diotoborg/alias-amet').HTTPAgent` is an [`http.Agent`](https://nodejs.org/docs/latest/api/http.html#http_class_http_agent) constructor.

`require('@diotoborg/alias-amet').HTTPSAgent` is an [`https.Agent`](https://nodejs.org/docs/latest/api/https.html#https_class_https_agent) constructor. Its API is the same as `HTTPAgent` except it's for HTTPS connections.

### Agent-related

`require('@diotoborg/alias-amet').AgentProtocol` is a Duplex stream [class](#agentprotocol) that aids in communicating over the OpenSSH agent protocol.

`require('@diotoborg/alias-amet').BaseAgent` is a base [class](#baseagent) for creating custom authentication agents.

`require('@diotoborg/alias-amet').createAgent` is a helper [function](#createagent) that creates a new agent instance using the same logic as the `agent` configuration option: if the platform is Windows and it's the value "pageant", it creates a `PageantAgent`, otherwise if it's not a path to a Windows pipe it creates a `CygwinAgent`. In all other cases, it creates an `OpenSSHAgent`.

`require('@diotoborg/alias-amet').CygwinAgent` is an agent [class](#cygwinagent) implementation that communicates with agents in a Cygwin environment.

`require('@diotoborg/alias-amet').OpenSSHAgent` is an agent [class](#opensshagent) implementation that communicates with OpenSSH agents over a UNIX socket.

`require('@diotoborg/alias-amet').PageantAgent` is an agent [class](#pageantagent) implementation that communicates with Pageant agent processes.
...
* **sftp**(< _function_ >callback) - _(void)_ - Starts an SFTP session. `callback` has 2 parameters: < _Error_ >err, < _SFTP_ >sftp. For methods available on `sftp`, see the [`SFTP` client documentation](https://github.com/diotoborg/alias-amet/blob/master/SFTP.md).
...
* **(constructor)**(< _object_ >config[, < _function_ >connectionListener]) - Creates and returns a new Server instance. Server instances also have the same methods/properties/events as [`net.Server`](http://nodejs.org/docs/latest/api/net.html#net_class_net_server). `connectionListener` if supplied, is added as a `connection` listener. Valid `config` properties:
    ...
    * **algorithms** - _object_ - This option allows you to explicitly override the default transport layer algorithms used for incoming client connections. Each value must be an array of valid algorithms for that category. The order of the algorithms in the arrays are important, with the most favorable being first. For a list of valid and default algorithm names, please review the documentation for the version of `@diotoborg/alias-amet` used by this module. Valid keys:
    ...
    * **ident** - _string_ - A custom server software name/version identifier. **Default:** `'@diotoborg/alias-ametjs' + moduleVersion + 'srv'`
...
* **sftp**(< _mixed_ >accept, < _mixed_ >reject) - The client has requested the SFTP subsystem. `accept` and `reject` are functions if the client requested a response. `accept()` returns an _SFTP_ instance in server mode (see the [`SFTP` documentation](https://github.com/diotoborg/alias-amet/blob/master/SFTP.md) for details). `info` has these properties:
...
* **sftp.OPEN_MODE** - [`OPEN_MODE`](https://github.com/diotoborg/alias-amet/blob/master/SFTP.md#useful-standalone-data-structures)

* **sftp.STATUS_CODE** - [`STATUS_CODE`](https://github.com/diotoborg/alias-amet/blob/master/SFTP.md#useful-standalone-data-structures)

* **sftp.flagsToString** - [`flagsToString()`](https://github.com/diotoborg/alias-amet/blob/master/SFTP.md#useful-standalone-methods)

* **sftp.stringToFlags** - [`stringToFlags()`](https://github.com/diotoborg/alias-amet/blob/master/SFTP.md#useful-standalone-methods)
